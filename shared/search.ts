...
export function taskMatches(task: Task, query: string, state?: Workspace): boolean {
  ...
  const haystack = normalizeSearch(
    [task.id, task.title, task.description, project, member, ...task.tags].join(' ')
  );
  return words.every((word) => haystack.includes(word));
}
export function searchTasks(state: Workspace, query: string): Task[] {
  return state.tasks.filter((task) => taskMatches(task, query, state));
}
