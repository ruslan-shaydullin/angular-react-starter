...
export function duplicateTask(state:Workspace,id:string,now:string):Result<Workspace> {
 ...
 const copy:Task={...task,id:newId,title:(task.title+' (copy)').slice(0,120),status:'backlog',archived:false,tags:[...task.tags],dependsOn:[],comments:[],timeEntries:[],checklist:task.checklist.map((item,index)=>({id:newId+'-check-'+(index+1),text:item.text,done:false})),createdAt:now,updatedAt:now};
