...
export function workspaceMetrics(state: Workspace, today: string) {
  ...
  return {
    ...
    overdue: tasks.filter((task) => dueState(task, today) === 'overdue').length,
    unassigned: tasks.filter((task) => !task.assignee && task.status !== 'done').length,
    blocked: tasks.filter(
      (task) =>
        task.status !== 'done' &&
        task.dependsOn.some((id) => state.tasks.find((item) => item.id === id)?.status !== 'done')
    ).length,
