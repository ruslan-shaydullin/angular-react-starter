...
export function validateTask(input: unknown, state: Workspace): Result<TaskDraft> {
  ...
  if (value.title.length < 3 || value.title.length > 120)
    errors.push('Title must contain 3 to 120 characters.');
  if (value.description.length > 2000)
    errors.push('Description must contain at most 2,000 characters.');
  ...
  if (!state.projects.some((project) => project.id === value.projectId))
    errors.push('Choose an existing project.');
  if (value.assignee && !state.members.some((member) => member.id === value.assignee))
    errors.push('Choose an existing team member.');
  if (value.dueDate && !validDate(value.dueDate)) errors.push('Use a valid calendar date.');
  if (!Number.isInteger(value.estimate) || value.estimate < 0 || value.estimate > 100)
    errors.push('Estimate must be a whole number from 0 to 100.');
