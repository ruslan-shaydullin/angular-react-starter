...
export function archiveTask(state: Workspace, id: string, archived: boolean): Result<Workspace> {
  ...
  if (
    archived &&
    state.tasks.some(
      (other) => !other.archived && other.status !== 'done' && other.dependsOn.includes(id)
    ) &&
    task.status !== 'done'
  )
    return failure(state, 'An active task still depends on this unfinished task.');
...
export function archiveSummary(state: Workspace) {
  ...
  return {
    ...
    completed: archived.filter((task) => task.status === 'done').length,
    ids: archived.map((task) => task.id)
  };
}
