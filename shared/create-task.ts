...
export function createTask(state:Workspace,input:unknown,now:string):Result<Workspace> {
 ...
 const task:Task={...parsed.value,id:nextId('T',state.tasks),tags:[],dependsOn:[],checklist:[],comments:[],timeEntries:[],archived:false,createdAt:now,updatedAt:now};
 return success({...state,tasks:[...state.tasks,task]});
}
export function blankTask(state:Workspace) {
 return {title:'',description:'',status:'backlog',priority:'normal',projectId:state.projects[0]?.id||'',assignee:'',dueDate:'',estimate:0};
}
