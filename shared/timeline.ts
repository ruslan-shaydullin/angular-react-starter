...
export function weekDates(anchor: string, weekStartsOn: 0 | 1): string[] {
  ...
  const day = new Date(anchor + 'T00:00:00Z').getUTCDay();
  const start = shiftDate(anchor, -((day - weekStartsOn + 7) % 7));
  return Array.from({ length: 7 }, (_, index) => shiftDate(start, index));
}
export function taskTimeline(state: Workspace): { date: string; tasks: Task[] }[] {
  const tasks = state.tasks.filter(
    (task) =>
      !task.archived && task.dueDate && (state.preferences.showCompleted || task.status !== 'done')
  );
