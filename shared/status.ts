...
export function transitionTask(state: Workspace, id: string, status: Status): Result<Workspace> {
  ...
  if (status === 'done') {
    const incomplete = task.dependsOn.filter(
      (id) => !state.tasks.some((item) => item.id === id && item.status === 'done')
    );
    if (incomplete.length)
      return failure(
        state,
        'Complete dependencies before finishing this task: ' + incomplete.join(', ')
      );
    if (task.checklist.some((item) => !item.done))
      return failure(state, 'Complete every checklist item before finishing this task.');
  }
