...
export function setEstimate(state: Workspace, id: string, estimate: number): Result<Workspace> {
  ...
  if (!Number.isInteger(estimate) || estimate < 0 || estimate > 100)
    return failure(state, 'Estimate must be a whole number from 0 to 100.');
...
export function estimateSummary(tasks: readonly Task[]) {
  ...
  const total = active.reduce((sum, task) => sum + task.estimate, 0);
  ...
  return {
    ...
    unestimated: active.filter((task) => task.estimate === 0 && task.status !== 'done').length,
