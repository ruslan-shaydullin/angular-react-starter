...
import { blankTask } from './shared/create-task';
import { PRIORITIES } from './shared/types';
import { TASK_TEMPLATES, taskFromTemplate } from './shared/templates';
export default function TaskEditor({ task, onClose }) {
  const { state, dispatch } = useWorkshop();
  const [draft, setDraft] = useState(() => (task ? { ...task } : blankTask(state)));
    ...
    <section className="panel" aria-labelledby="editor-title">
      ...
      {!task && (
        <label>
          ...
          <select
            defaultValue=""
            onChange={(event) => setDraft(taskFromTemplate(state, event.target.value))}
          >
        ...
        <div className="row">
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => update('estimate', Number(event.target.value))}
            />
          </label>
        </div>
        <p className="muted">
          Title needs 3–120 characters. Estimates use whole points from 0 to 100.
        </p>
        <div className="row">
