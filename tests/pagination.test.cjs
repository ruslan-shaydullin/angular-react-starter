...
test('empty lists retain a navigable first page', () => {
  assert.deepEqual(paginate([], 99, 6), {
    items: [],
    page: 1,
    pageCount: 1,
    total: 0,
...
test('clamps stale page after filters shrink result set', () => {
...
test('sanitizes invalid paging inputs', () => {
  assert.equal(paginate([1, 2, 3], NaN, NaN).page, 1);
  assert.equal(paginate([1, 2, 3], -1, 0).items.length, 1);
  assert.equal(
    paginate(
...
test('page windows stay within total page count', () => {
