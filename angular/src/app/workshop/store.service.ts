...
import { createWorkspace } from '../../workshop/shared/fixtures';
import { createHistory, commitCommand, undo, redo, History } from '../../workshop/shared/history';
...
export class WorkshopStore implements OnDestroy {
  ...
  filters = filtersFromHash(window.location.hash);
  selected: string[] = [];
  page = 1;
  ...
  constructor() {
    ...
    try {
      const loaded = loadWorkspace(window.localStorage, state);
      state = loaded.value;
      this.errors = loaded.errors;
    } catch {
      this.errors = ['Browser storage is unavailable.'];
    }
  ...
  private install(history: History, message: string): void {
    this.history = history;
    this.message = message;
    this.errors = [];
    this.selected = reconcileSelection(this.selected, history.present.tasks);
    try {
      ...
      if (!saved.ok) this.errors = saved.errors;
    } catch {
      this.errors = ['Browser storage is unavailable. Export a snapshot to keep changes.'];
    }
  }
  dispatch(type: string, payload?: unknown): boolean {
    const result = commitCommand(this.history, { type, payload }, new Date().toISOString());
    if (!result.ok) {
  ...
  undo(): void {
    this.install(undo(this.history), 'Change undone.');
  }
  redo(): void {
    this.install(redo(this.history), 'Change redone.');
  }
}
