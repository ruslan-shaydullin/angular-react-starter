...
import { keyboardCommand, SHORTCUTS } from '../../workshop/shared/keyboard';
...
export class KeyboardHelpComponent {
  ...
  @HostListener('window:keydown', ['$event']) onKey(event: KeyboardEvent): void {
    ...
    if (command === 'help') this.s.help = !this.s.help;
    if (command === 'undo') this.s.undo();
    if (command === 'redo') this.s.redo();
